...
export const listarTodos = async (req, res) => {
  try {
    ...
    if (status || prioridade || responsavel) {
      const chamados = await ChamadosModel.findWithFilters({
        status,
        prioridade,
        responsavel,
      });
...
export const listarUm = async (req, res) => {
  try {
    ...
    if (
      ...
      (responsavel && String(chamado.responsavelId) !== String(responsavel))
    ) {
      return res.status(404).json({
        erro: "Chamado encontrado, mas não corresponde aos filtros",
        id: id,
      });
    }
    return res.status(200).json({
  ...
  try {
    ...
    if (dado.criadoPorId !== undefined && dado.criadoPorId !== null) {
      ...
      if (!exists) {
        return res
          .status(400)
          .json({ erro: "criadoPorId não encontrado no banco" });
      }
    }
    ...
    if (dado.responsavelId !== undefined && dado.responsavelId !== null) {
      ...
      if (!exists) {
        return res
          .status(400)
          .json({ erro: "responsavelId não encontrado no banco" });
      }
    }
...
export const apagar = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ erro: "Erro ao apagar chamado", detalhes: error.message });
  }
};
...
export const listarStatus = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ erro: "Erro ao listar status", detalhes: error.message });
  }
};
...
export const listarPrioridade = async (req, res) => {
  try {
    ...
    if (prioridades.length === 0) {
      return res
        .status(404)
        .json({ mensagem: "Nenhuma prioridade encontrada!" });
    }
    ...
    return res
      .status(500)
      .json({ erro: "Erro ao listar prioridades", detalhes: error.message });
  }
};
...
export const listarResponsavel = async (req, res) => {
  try {
    const chamados = await ChamadosModel.findAll();
    const responsaveis = [
      ...new Set(chamados.map((c) => c.responsavelId).filter(Boolean)),
    ];

    if (responsaveis.length === 0) {
      return res
        .status(404)
        .json({ mensagem: "Nenhum responsável encontrado!" });
    }
    ...
    return res
      .status(500)
      .json({ erro: "Erro ao listar responsáveis", detalhes: error.message });
  }
};
