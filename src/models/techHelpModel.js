...
export const findWithFilters = async ({ status, prioridade, responsavel }) => {
  ...
  if (prioridade) {
    conditions.push({
      prioridade: { equals: String(prioridade), mode: "insensitive" },
    });
  }
...
export const create = async (data) => {
  ...
  const criadoPorId =
    data.criadoPorId !== undefined && data.criadoPorId !== null
      ? Number(data.criadoPorId)
      : null;
  const responsavelId =
    data.responsavelId !== undefined && data.responsavelId !== null
